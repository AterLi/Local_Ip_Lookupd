...
# Excel headers for output
EXCEL_HEADERS = ["IP", "ISP", "Hostname", "Region", "Country_code"]
...
def write_to_excel(save_directory, partial=False, has_count=False):
    ...
    filename = f'ipinfo_export-{now}{"_PARTIAL" if partial else ""}.xlsx'
...
def process_file(file_path, save_directory, in_process_window, root):
    ...
    if result_list:
        save_path = write_to_excel(save_directory, partial=api_limit_hit, has_count=has_count)
...
def process_single_ip(ip, save_directory, in_process_window, root):
    ...
    if api_limit_hit:
        messagebox.showerror("API Limit Reached",
                             "The API rate limit has been reached.\nPlease try again later.")
        return

    if result_list:
        save_path = write_to_excel(save_directory)
        if messagebox.askyesno("Success", f"Results saved at: {save_path}\nDo you want to close the program?"):
            root.quit()
            root.destroy()
    else:
        messagebox.showerror("Error", "Failed to scan the IP address. No file was saved.")


def process_multiple_ips(ip_list, save_directory, in_process_window, root):
    """Process a list of IPs entered manually."""
    global result_list
    result_list = []
...
class IPCheckerApp:
    ...
    def _create_widgets(self):
        ...
        # Single IP
        tk.Label(frame, text="Or enter a single IP address:",
                 font=self.font).pack(pady=5)
        tk.Button(frame, text="Enter Single IP", command=self._input_single_ip,
                  font=self.font).pack(pady=5)

        # Multiple IPs
        tk.Label(frame, text="Or enter multiple IP addresses:",
                 font=self.font).pack(pady=5)
        tk.Button(frame, text="Enter Multiple IPs", command=self._input_multiple_ips,
                  font=self.font).pack(pady=5)
        ...
        if file_path:
            ...
            if save_directory:
                ...
                run_in_thread(process_file, file_path, save_directory,
                             in_process_window, self.root)

    def _input_single_ip(self):
        """Open single IP input dialog."""
        def on_submit():
            ...
            if ip:
                ...
                if save_directory:
                    ...
                    tk.Label(in_process_window, text="In process... Please wait",
                             font=("Helvetica", 14)).pack(expand=True)
                    run_in_thread(process_single_ip, ip, save_directory,
                                 in_process_window, self.root)
            ip_input_window.destroy()

        ip_input_window = tk.Toplevel(self.root)
        ip_input_window.title("Enter IP Address")
        ip_input_window.geometry("300x150")
        tk.Label(ip_input_window, text="Enter IP address:",
                 font=("Helvetica", 14)).pack(pady=10)
        ip_entry = tk.Entry(ip_input_window, font=("Helvetica", 14))
        ip_entry.pack(pady=10)
        tk.Button(ip_input_window, text="Submit", command=on_submit,
                  font=("Helvetica", 12)).pack(pady=10)

    def _input_multiple_ips(self):
        """Open multiple IP input dialog."""
        def on_submit():
            ...
            if save_directory:
                ...
                tk.Label(in_process_window,
                         text=f"Scanning {len(ip_list)} IPs...\nPlease wait",
                         font=("Helvetica", 14)).pack(expand=True)
                run_in_thread(process_multiple_ips, ip_list, save_directory,
                             in_process_window, self.root)
        ...
        ip_input_window = tk.Toplevel(self.root)
        ip_input_window.title("Enter Multiple IP Addresses")
